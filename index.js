  ...
  try {
    ...
    app.get('/user', async (req, res) => {
      try {
        ...
        const AllUsersForAdmin= await users.toArray();
        res.send(AllUsersForAdmin);
      } catch (error) {
    ...
    // GET: Fetch paginated jobs with optional category filter
    app.get('/jobs', async (req, res) => {
      try {
        const page = parseInt(req.query.page) || 1; // default to page 1
        const limit = parseInt(req.query.limit) || 9; // default 9 jobs per page
        const category = req.query.category || ""; // NEW: category from query
        const skip = (page - 1) * limit;

        let filter = {};
        if (category && category !== "all") {
          filter.category = category; // assuming job documents have a "category" field
        }

        const jobs = await AllServices.find(filter)
          .skip(skip)
          .limit(limit)
          .toArray();

        const total = await AllServices.countDocuments(filter);
